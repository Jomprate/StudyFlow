import './SubjectStudentId.css';
import { deleteSubject, getSubjectByTeacher } from '../../utils/subject';
...
const SubjectId = () => {
  ...
  useEffect(() => {
    getSubjectByTeacher({ teacherId }).then((response) => setData(response));
  }, [teacherId]);
  ...
  return (
    <div className="subject-id">
      <p className="subject-id-tittle">Tareas del profesor</p>
      {getTable()}
    </div>
  );
};
